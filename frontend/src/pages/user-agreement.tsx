...
const sections: LegalSection[] = [
  {
    ...
    title: "协议适用范围",
    content: [
      "本协议适用于您访问、注册、登录或使用 量子平台 智能开发平台及其相关网站、控制台、工具和服务的全部行为。",
  ...
  },
  {
    ...
    items: [
      ...
      "利用 量子平台 制造、传播或协助开发带有网络攻击、入侵破坏、恶意控制、漏洞利用等性质的软件、脚本或工具。",
      "直接使用 量子平台 或借助平台生成内容发起网络攻击、渗透破坏、恶意扫描、批量漏洞利用、拒绝服务攻击或其他危害网络与信息系统安全的行为。",
      "以盗取、冒用、转售、共享、绕过限制或其他未经授权的方式使用 量子平台 的 API Token、账号能力或平台资源，并将其提供给其他 AI 工具、脚本、服务或第三方继续调用。",
  ...
  },
  {
    ...
    items: [
      ...
      "前述可导致封号或其他处置的行为，包括但不限于：使用 量子平台 制造带有网络攻击性质的软件，直接使用 量子平台 发起网络攻击，盗取或冒用 量子平台 API Token 供其他 AI 工具继续调用，以及通过批量注册、作弊邀请、脚本刷量等恶意行为盗刷邀请注册积分或其他平台权益。",
    ],
  },
  {
    id: "contact",
  ...
  return (
    <AuthProvider>
      <LegalTerminalPage
        ...
        title="用户协议"
        subtitle="本协议用于说明您在使用 量子平台 及相关服务时应遵守的规则，以及平台与您之间的权利义务关系。请在使用前认真阅读。"
